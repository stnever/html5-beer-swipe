...
function init({windowHeight, windowWidth, maxRows, maxCols}) {
  tileSize = Math.floor((windowHeight - (2 * outer$1)) / maxRows);
  console.log('tile size is %spx', tileSize);

  boardSize = [
    (2*outer$1) + (maxCols * (tileSize)),
    (2*outer$1) + (maxRows * (tileSize))
  ];
}
...
function getPosition(row, col) {
  var x = outer$1 + col * (tileSize + inner),
      y = outer$1 + row * (tileSize + inner);
  return [x,y]
}

function xyToBoard(x, y) {
  var col = Math.floor((x - outer$1) / (tileSize + inner)),
      row = Math.floor((y - outer$1) / (tileSize + inner));
  return [row,col]
}
...
class Sprite {
  ...
  _redraw() {
    if ( lodash.isFunction(this.contents) ) {
      this.$el.innerHTML = this.contents();
    } else if ( lodash.isString(this.contents) ) {
      this.$el.innerHTML = this.contents;
    }
...
class Board extends Sprite {
  constructor(opts={}) {
    ...
    this.cells = lodash.times(stage.maxRows, row => {
      return lodash.times(stage.maxCols, col => {
  ...
  walk(fn) {
    lodash.forEach(this.cells, (row, i) => {
      lodash.forEach(row, (cell, j) => fn(cell, i, j));
    });
  }
...
class Stage {
  ...
  trySwap(from, to) {
    var isValid = lodash.find(this.possibleSwaps, swap => {
  ...
  detectPossibleSwaps() {
    ...
    lodash.forEach(this.possibleSwaps, s => console.log('possible swap: %s => %s', s.posA.join(), s.posB.join()));
  }
